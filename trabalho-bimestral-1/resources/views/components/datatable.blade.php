                ...
                <tr>
                    @foreach($header as $head)
                        ...
                        @if($head == "status")
                            ...
                            @endif
                        @elseif($head == "eixo")
                            @foreach($data['dados2'] as $itemAux)
                                @if($item['eixo_id'] == $itemAux['id'])
                                    <td>{{ $itemAux['nome'] }}</td>
                                @endif
                            @endforeach
                        @elseif($head == "curso")
                            @foreach($data['dados2'] as $itemAux)
                                @if($item['curso_id'] == $itemAux['id'])
                                    <td>{{ $itemAux['nome'] }}</td>
                                @endif
                            @endforeach
                        @else
                    ...
                    <td class="d-flex flex-row-reverse ">
                        ...
                        <a nohref style="cursor:pointer" onclick="showInfoModal('  {{$item}}    ')" class="btn btn-primary m-1">
